                    ...
                    <div className="lg:w-8/12 w-full flex max-lg:flex-wrap lg:pt-0 pt-4  items-center max-lg:justify-center lg:gap-[71px] md:gap-16 sm:gap-12 gap-8">
                        {KICKBOX_DATA_LIST.map((item, index) => (
                            <div className="max-w-max max-lg:text-center" key={index} >
                                <p className="xl:text-[80px] lg:text-6xl max-sm:min-w-56 md:text-5xl sm:text-4xl text-3xl text-yellow">{item.number}</p>
                                <p className="text-lightBlack font-bold lg:ps-1 xl:text-[21px] xl:mt-0.5 md:text-lg sm:text-base text-sm">{item.content}</p>
                            </div>
                        ))}
                    </div>
                    <div className="lg:w-4/12 w-full flex justify-center lg:pt-0 pt-8 items-center">
                        <CustomButton customStyles='text-red hover:!text-white hover:bg-red !border-red xl:max-w-[268px] max-w-[240px] max-lg:!py-2 w-full xl:h-[56px]' title='Submit your idea!' />
                    </div>
                </div>
