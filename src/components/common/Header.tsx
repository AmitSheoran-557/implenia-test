    ...
    return (
        <div className="bg-white mx-auto w-full flex justify-center items-center">
            <div className="flex justify-between items-center mx-auto w-full max-w-[1212px] px-4 rounded-[20px] overflow-hidden lg:py-[18px] md:py-4 py-2.5">
                <button aria-label="menu" onClick={handleOpen} className={`hidden size-7 justify-center max-lg:absolute max-lg:right-4 relative z-[70] max-lg:flex flex-col overflow-hidden`}>
                ...
                <Link href="/">
                    <img className="xl:max-w-[93px] xl:min-w-[93px] lg:max-w-20 lg:min-w-20 md:max-w-16 max-w-12 w-full" src="/assets/images/webp/header-logo.webp" alt="page-logo" />
                </Link>
                <div className={`flex xl:gap-[30px] max-lg:px-4 bg-white relative w-full mx-auto justify-center lg:gap-7 gap-6 !text-black lg:max-h-max max-lg:fixed max-lg:top-0 max-lg:h-full max-lg:w-full max-lg:flex-col max-lg:bg-white bg-cover bg-center max-lg:duration-300 max-lg:justify-center max-lg:items-center z-[60] ${open ? "max-lg:left-0" : "max-lg:left-full"}`}>
                    {HEADER_DATA_LIST.map((item, index) => (
                        <Link onClick={handleOpen} href={item.path} className="text-black text-shadow lg:hidden max-lg:block font-bold lg:text-base text-sm transition-all duration-300 ease-linear hover:text-yellow" key={index}>{item.link}</Link>
                    ))}
                </div>
